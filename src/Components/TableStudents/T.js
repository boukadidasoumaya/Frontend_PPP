...
const toggleUpdateModal = (student) => {
    ...
    return (
        ...
        <Row>
          ...
          <UncontrolledDropdown>
            ...
            <DropdownMenu className="dropdown-menu-arrow" right>
              ...
              <Modal isOpen={updateModalOpen} toggle={() => toggleUpdateModal(student)}>
                    <ModalHeader toggle={() => toggleUpdateModal(null)}>Modify Student</ModalHeader>
                    <ModalBody>
                      {/* Form fields to capture updated student data */}
                      <FormGroup>
                        <FormLabel for="firstname">First Name</FormLabel>
                        <Input type="text" name="firstname" id="fistname" placeholder="Enter First Name" value={student ? student.FirstName : ''} onChange={handleChange} />
                      </FormGroup>
                      <FormGroup>
                        <FormLabel for="lastname">Last Name</FormLabel>
                        <Input type="text" name="lastname" id="lastname" placeholder="Enter Last Name" value={student ? student.LastName : ''} onChange={handleChange}/>
                      </FormGroup>
                      <FormGroup>
                        <FormLabel for="cin">Num CIN</FormLabel>
                        <Input type="text" name="cin" id="cin" placeholder="Enter CIN" value={student ? student.CIN : ''} onChange={handleChange}/>
                      </FormGroup>
                      <FormGroup>
                        <FormLabel for="email">Email</FormLabel>
                        <Input type="text" name="email" id="email" placeholder="Enter Email" value={student ? student.Email : ''} onChange={handleChange}/>
                      </FormGroup>
                      <FormGroup>
                        <FormLabel for="birthday">Birthday</FormLabel>
                        <Input type="date" name="birthday" id="birthday" placeholder="Enter Date of Birth" value={student ? student.Birthday : ''} onChange={handleChange} />
                      </FormGroup>
                      <FormGroup>
                        <FormLabel for="major">Major</FormLabel>
                        <select className="form-control shadow-none border-1 bg-transparent text-dark" name="major" id="major" value={student ? student.Major : ''} onChange={handleChange}>
                          <option value="">Select Major</option>
                          {majors.map((major) => (
                            <option key={major} value={major}>
                              {major}
                      ...
                      <FormGroup>
                        <FormLabel for="level">Level</FormLabel>
                        <select className="form-control shadow-none border-1 bg-transparent text-dark" name="level" id="level" value={student ? student.Year : ''} onChange={handleChange}>
                          <option value="">Select Level</option>
                          {levels.map((level) => (
                            <option key={level} value={level}>
                              {level}
                      ...
                      <FormGroup>
                        <FormLabel for="group">Group</FormLabel>
                        <select className="form-control shadow-none border-1 bg-transparent text-dark" name="group" id="group" value={student ? student.Group : ''} onChange={handleChange}>
                          <option value="">Select Group</option>
                          {groups.map((group) => (
                            <option key={group} value={group}>
                              {group}
        ...
        <div className="modal-footer">
          <Button className='addbtn' onClick={()=>{handleDelete(student)}}>Delete</Button>
